...
기존 작업자의 직관에 의존한 재배치를 효율적으로 전환하기 위해 
공공자전거 대여소와 이력 데이터를 수집·정제·가공하여
데이터 파이프라인과 최적화 시스템을 구축한 프로젝트

Raw Data → ETL → Net Demand → Target Quantity → Pick/Drop Selection → Optimization Dataset

(기존 [재배치 방식]은 [작업자의 직관에 의존한 작업이라 비효율적]이라는 문제가 있었습니다. 이를 해결하기 위해 [데이터 엔지니어]로서 [대여소별 클러스터링, 정수선형계획법을 통한 재고 이동과 차량경로문제 기반의 방문 순서 최적화] 방식을 제안하고 구현했습니다. 결과적으로 []를 달성하여 [목표 재고량 70%의 달성]를 이끌어냈습니다.)

## 핵심 기능
- TASHU Open API 기반 데이터 수집 파이프라인 구축
- 순수요(Net Demand) 기반 Pick / Drop 대여소 자동 선정
- ILP + VRP 기반 재배치 수량 및 경로 최적화

## Tech Stack
|기술|용도|
|-----|-----|
| Python(3.8)| 데이터 처리 및 최적화 로직 | 
| Pandas() | 데이터 정제 및 가공 |
| NumPy() | 연산 |
| Scikit-Learn() | 대여소 클러스터링 | 
| PuLP() | ILP, VRP 문제 최적화 |
| Folium() | 지도 기반 시각화 |
| Matplotlib() | 결과 시각화 |
...
- API 데이터 수집 및 정제
- ETL 파이프라인 구축
- 순수요(Net Demand) 데이터 가공 모델링
- Pick/Drop 선정 알고리즘 구현
- 최적화 입력 데이터 생성
- 재배치 성능 평가
...
재배치 의사결정에 활용 가능한 데이터 파이프라인을 구축한 프로젝트
