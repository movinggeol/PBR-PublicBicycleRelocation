...
import pandas as pd
import numpy as np
# 형제 모듈. 두 경로를 다 연다 — 자세한 이유는 pipeline/step2_optimize/vrp.py 참고(1.26.154).
try:
    ...
    from pipeline.step1_cluster.adjust_module import (
        compute_medoids, compute_objective, select_cluster_candidates,
        make_cluster_pairs, get_movable_nodes, check_size_constraint, try_move_node)
...
def select_top_unbalanced_st(file_path:str, duration:str, st_info:pd.DataFrame) -> pd.DataFrame:
    ...
    #
    # 🔴 **다만 호출부가 자료를 직접 건네면 그것을 쓴다** (2026-09-12에 물렸다).
    # 실험 하네스(baseline_compare 등)는 자기가 고른 `z`로 재배치량을 계산해
    # **메모리 버퍼**로 넘긴다. 그런데 위 DB 조회의 `now`는 **모듈을 불러오는
    # 순간 고정된 기본 라벨**이라 실험이 고른 스냅샷과 무관하다. 그래서
    #   · DB에 그 라벨·회차가 있으면  → 건네받은 버퍼를 **조용히 무시**하고
    #                                   엉뚱한 자료로 계산했다(_05_10이 그랬다)
    #   · 없으면                      → Path(StringIO)에서 TypeError로 죽었다
    # 둘 다 **실험 결과가 거짓이 된다.** 조용한 쪽이 더 나쁘다 — 표가 그럴듯하게
    # 찍히기 때문이다. 건네준 자료가 있으면 그것이 정답이다.
    if hasattr(file_path, 'read'):
        st_rebal = pd.read_csv(file_path)
    else:
