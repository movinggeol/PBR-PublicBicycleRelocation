...
| `docs/구현/TESTING.md` | 테스트 746개가 지키는 것·격리 장치·외부 API 수동 검증 (**테스트 추가 전 필독**) |
...
- **의도적으로 근사·단순화를 남겨둘 때는 `ponytail:` 주석으로 한계와 다음 수를
  같이 적는다** — 코드 안에 있는 짧은 경고가 문서를 열어야만 아는 함정보다
  낫다(외부 규칙 ponytail의 관례를 이 저장소 용도로 가져옴). 이 저장소에서
  이미 근사인 채로 쓰는 것들: `improvement_rate`(계획 달성률이지 실제 효과가
  아님, KPI.md), `TARGET_Z`·`CLUSTER_GAMMA`(재현 실험으로만 갱신, 위 항목),
  이동시간 계수(10일 미만 표본에서는 변동계수를 정의상 못 잰다, EXPERIMENTS
  9장). 새로 이런 근사를 도입하면 코드에도 `# ponytail: <한계> — <다음 수>`
  한 줄을 남긴다.
