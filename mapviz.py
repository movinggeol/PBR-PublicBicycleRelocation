...
PICK_COLOR = "#0072B2"   # 수거 — 자전거가 남는 곳에서 실어 온다
DROP_COLOR = "#D55E00"   # 배송 — 자전거가 모자란 곳에 내린다

# 화면·종이·지도가 같은 말을 쓰게 한다. 예전 범례는 `Drop`·`Pick`이라는
# 영어를 그대로 노출했는데, 지시서는 수거/배송, CSV는 pick/drop이라
# **한 개념에 용어가 세 벌**이었다.
...
def swatch_circle_dashed(color: str = "#8a8a8f") -> str:
    ...
    같아진다 — 크기로는 영영 구분되지 않는다. 색은 이미 수거/배송을 뜻하고
...
# 화면의 군집 지도에는 범례가 아예 없었고, 1.26.107이 수거·배송 색을 웹과
