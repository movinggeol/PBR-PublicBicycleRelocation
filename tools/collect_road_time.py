...
패널은 `tools/road_panel.json`에 남아 **커밋으로 나르고**(1.26.52), 있으면 다시
만들지 않는다 — 수집 도중에 패널이 바뀌면 그때까지 쌓은 것이 비교 불가능해진다.
옛 자리(`data/road_panel.json`)에만 있으면 처음 읽을 때 옮겨 온다.

## 시각 처리

`startTime`은 **그 시각의 교통량**을 정한다. 파이프라인과 똑같이
`start_time_for(duration, day_type)`(평일이면 다음 평일, 휴일이면 다음 토·일·
공휴일 + 회차 첫 시각)을 쓴다 — 우리가 검증하려는
것이 파이프라인이 실제로 소비하는 값이기 때문이다. 따라서 이 수집기는
**하루 중 아무 때나 돌려도 된다**(재고 수집기와 달리 창 가드가 없다).

⚠️ 평일과 휴일은 `day_type`으로 **따로 잰다**(1.26.158). `--day-type`을 주지
않으면 그날의 실제 요일로 정한다 — 평일이면 weekday, 주말·공휴일이면 holiday다.
설치 스크립트는 전요일로 깨운다(1.26.159).
...
저장은 `road_leg`, `run_label`은 평일 `roadprobe-YYYY-MM-DD` · 휴일
`roadprobe-holiday-YYYY-MM-DD`다. 같은 날 다시 돌리면
...
그래서 **하루에 여러 번 깨우되 필요한 때만 호출**하게 했다. 이 옵션은 셋을 본다.
  ...
  1. **그날의 요일로 평일·휴일을 갈라 잰다** (`--day-type`을 주면 그쪽을 따른다).
     주말·공휴일에는 휴일 계수를 쌓는다 — 1.26.158 전에는 주말이면 아무것도 하지
     않았는데, `start_time_for()`가 '다음 평일'만 만들어 금·토·일이 같은 값을
     가리켰기 때문이다. 지금은 휴일이면 '다음 토·일·공휴일'을 만든다.
