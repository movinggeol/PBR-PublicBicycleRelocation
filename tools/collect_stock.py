...
def keep_awake() -> None:
    """시스템 유휴 타이머를 리셋하려고 부른다 — **모던 대기 PC에서는 절전을 못 막는다.**

    처음에는 *"10분마다 도는 틱이 절전 임계값을 계속 밀어낸다"* 고 믿고 넣었다.
    집환경(모던 대기) 실측은 반대였다(2026-09-22, 1.26.276). 09-14에 전원을 꽂은 채
    틱 30개가 모두 이것을 불렀는데도 화면이 꺼지고 정확히 296분(설정 5시간) 뒤 절전
    단계에 들었고, 그 뒤로 틱이 끊겼다. 배터리에서도 같다(09-22, 설정 4시간). 수집을
    지키는 것은 이 호출이 아니라 **전원 설정**이다 — 전원 연결 시 절전 '안 함'
    (docs/구현/COLLECTOR.md 7장).

    다른 기기에서는 도울 수 있고 해가 없어 남겨 둔다. 실패해도 수집은 그대로 진행한다.
    """
...
def uptime_blocks(stamps: Sequence[datetime], interval: int) -> list:
    ...
    이 함수만으로는 가릴 수 없다 — 스크립트가 아예 실행되지 않았거나(전원·절전·
    로그오프·스케줄러 거부), 돌았는데 연결이 안 됐거나다.
...
def failure_ticks(log_path: Optional[Path] = None) -> dict:
    ...
    남기지만(그래서 격자가 필요하다), **돌았는데 연결이 안 된 것은 남긴다.**
    둘을 겹쳐야 결측의 원인이 갈린다.

    ⚠️ **여기서 갈리는 것은 "스크립트가 돌았나"까지다.** 연결이 왜 안 됐는지
    (원격 장애냐 이쪽 망이냐)는 `ConnectionError` 하나로는 알 수 없다 —
    2026-09-12의 20틱은 **와이파이 끊김**이었다(사용자 확인, 1.26.183).
...
def coverage(start: clock, end: clock, interval: int) -> pd.DataFrame:
    ...
    **결측은 로그가 아니라 DB의 기대 격자와 대조한다** — 절전으로 놓친 틱은
    ...
    있고 **연결이 끊겨 있었다는 뜻일 수도 있다** — `실패`가 그 둘을 가른다.
...
def print_status(start: clock, end: clock, interval: int,
    ...
    #    ⚠️ 그렇다고 **상대 탓으로 갈아타지도 않는다** — 그날 원인은 타슈가
    #    아니라 이쪽 와이파이였다(1.26.183).
    ...
    if not split.empty:
        ...
        if blamed:
            ...
            print("             PC가 아니라 연결이 끊긴 구간이 섞여 있습니다.")
        if len(split) - blamed:
            print(f"             나머지 {len(split) - blamed}일은 수집기가 아예 실행되지")
            print("             않은 구간입니다 — PC 꺼짐·절전(모던 대기)·로그오프·")
            print("             스케줄러 거부 중 하나입니다. 연결 실패는 아닙니다.")
    ...
    if not failed.empty:
        ...
        print("             스크립트는 돌았는데 연결이 안 됐습니다 — 원격 장애일")
        print("             수도, 이쪽 망(와이파이)이 끊긴 것일 수도 있습니다.")
        print("             켜 두는 것만으로는 부족하고 망이 붙어 있어야 합니다.")
        print("             휴일이면 회복에 일주일이 걸립니다.")
    print()
