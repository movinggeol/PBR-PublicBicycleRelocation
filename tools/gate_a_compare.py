...
- ⚠️ **변화 0인 작업도 정보다.** 이동시간을 거치지 않는 작업이면 같아야 한다 — 25장
  `fleet_outage_stress`는 회차당 군집 수를 12장 실측 `[12, 14, 16]`으로 박아 두어 **같게 나오는 것이
  정상**이다(대신 켠 채 12장의 K가 달라지면 그 상수를 고쳐 다시 돌린다, 수집완료_계획 2-5 정정).
  반대로 **거쳐야 할 작업이 0이면** 스위치가 그 경로에 닿지 않는다는 뜻이라 먼저 의심한다.
- 요약의 exit 칸이 `timeout`이면 `gate_a_rerun.ps1`이 시간 제한(`-JobTimeoutMin`)으로 끊은 작업이다 —
  실패로 세고, `-Only`로 다시 돌린 폴더를 `--on-also`/`--off-also`로 준다.
