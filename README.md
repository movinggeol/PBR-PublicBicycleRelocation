...
| --- | --- | --- | --- |
| 무재배치 | 2.40 | 1.76 | 1.64 |
| 그리디 (군집·ILP 없음) | 1.15 | 0.41 | 0.60 |
| 지리 균등 군집 (불균형 조정 없음) | 1.83 | 1.46 | 1.24 |
| 평균 목표재고 (`z = 0`) | 1.55 | — | — |
| **제안 방법** | **1.05** | **0.34** | **0.50** |

> **2026-09-07 재측정 · 씨앗 3개(42·7·13) 평균.** 스냅샷은 실행 라벨
> `2026-08-11 real`(대여소 1,368곳·총재고 3,214대·거치대 9,713면, 지문
> `9e4c0d5c`)입니다. **결품의 절댓값은 이 스냅샷에 딸려 있습니다** — 이전 판
> (2026-08-26)의 스냅샷은 DB에 남아 있지 않아 재현되지 않았습니다. 경과는
> [EXPERIMENTS.md](docs/분석/EXPERIMENTS.md) 31장, 논문 표는
> [6.3](docs/연구/초안/6장_실험_성능평가.md)입니다.
> `z = 0`은 `_10_15`·`_15_20`에서 **작업 대상을 하나도 만들지 못해** 값이 없습니다.
>
> ✅ **12개월 반복으로 유의성을 확인했습니다** — 무재배치·그리디 대비 세 회차
> **각각** 유의합니다(Wilcoxon 부호순위, 12개월 × 씨앗 3개, 각 `n = 36`,
> `p < 10⁻⁵`). 지리 균등 군집과는 5개월(`n = 15`, `p = 0.0001`) 기준입니다.

> **그리디는 제안 방법보다 더 많이 옮기고도(회차당 216~328대 vs 201~299대)
> 결품을 더 남깁니다.** 몇 대를 옮기느냐가 아니라 어디로 옮기느냐의 문제입니다.
> 다만 **km당 편익은 그리디와 대등합니다** — 이동거리는 오히려 제안 방법이
> 깁니다(375~459km vs 322~373km). 제안 방법의 우위 중 일부는 더 멀리 다녀서
> 얻은 것입니다.

> **재배치는 공짜가 아닙니다** — 결품을 줄인 만큼 거치대가 차서 반납이 막힙니다
> (포화 시간 +0.19~0.44h). 편익이 3~6배 커서 순위는 바뀌지 않지만, 두 지표를
> 나란히 싣습니다.

> **`z = 0`은 손댈 곳 자체를 못 찾습니다** — `_05_10`에서 2.40 → 1.55h로
> 개선이 그치고, 나머지 두 회차는 작업 대상이 0곳입니다. 안전재고의 값어치는
> 손해를 막는 데가 아니라 **무엇을 손댈지 정하는 데** 있습니다.

**하루 3회차 운용 결과** (05~10 / 10~15 / 15~20시)

| 회차 | 처리 | 이동거리 | 최장 소요 | 예산 초과 | 결품 시간 (전 → 후) |
| --- | --- | --- | --- | --- | --- |
| `_05_10` | 299대 | 458.5 km | 120.3분 | 0.3건 | 2.40h → **1.05h** |
| `_10_15` | 201대 | 374.7 km | 115.5분 | 0.3건 | 1.76h → **0.34h** |
| `_15_20` | 205대 | 383.0 km | 109.9분 | 0건 | 1.64h → **0.50h** |
| **합계** | **705대** | **1,216 km** | — | **0.7건** | — |
...
※ 25년 11월 순수요(평일), **씨앗 3개(42·7·13) 평균**, 스냅샷 `2026-08-11 real`, **2026-09-07 재측정** 결과입니다. 예산 초과가 정수가 아닌 것은 세 씨앗의 평균이기 때문입니다(0.3건 = 셋 중 한 번)
...
python -m pytest              # 750개 통과 확인 (약 100초)
...
python -m pytest                 # 750개, 약 100초 (tests/ 만 수집)
```

- `tests/test_pipeline.py` (45) — 합성 데이터로 step0→step1→step2→step4를
  **subprocess로 실제 실행**한 뒤 산출물 존재·스키마·ILP 공급 제약·개선량을 검증.
  실행마다 고유 라벨(`smoketest-{PID}`)을 써서 실데이터를 건드리지 않고,
  끝나면 그 라벨 파일만 정리합니다.
- `tests/test_webapp.py` (140) — 라우트·경로 탈출 차단·실행 폼 입력 검증,
  그리고 **화면이 실제로 무엇을 보여 주는가** — 명암비·400% 확대·인쇄·빈 상태,
  화면이 말하는 범위와 실제로 센 범위가 같은지
- `tests/test_calculations.py` (77) — **계산 단위 테스트**: 목표재고 공식(`μ + zσ`,
...
| [docs/구현/TESTING.md](docs/구현/TESTING.md) | **테스트** — 750개가 무엇을 지키는지, 외부 API 수동 검증 절차 |
