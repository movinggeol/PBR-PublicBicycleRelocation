...
python -m pytest              # 793개 통과 확인 (약 100초)
...
python -m pytest                 # 793개, 약 100초 (tests/ 만 수집)
```
...
- `tests/test_pipeline.py` (46) — 합성 데이터로 step0→step1→step2→step4를
  **subprocess로 실제 실행**한 뒤 산출물 존재·스키마·ILP 공급 제약·개선량을 검증.
  ...
  끝나면 그 라벨 파일만 정리합니다.
- `tests/test_webapp.py` (163) — 라우트·경로 탈출 차단·실행 폼 입력 검증,
...
| [docs/구현/TESTING.md](docs/구현/TESTING.md) | **테스트** — 793개가 무엇을 지키는지, 외부 API 수동 검증 절차 |
