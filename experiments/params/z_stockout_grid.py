...
🔴 **결함이 있다 — 결론에 그대로 쓰지 마라 (1.26.64).** `z`가 `target_qty`를
바꾸므로 **후보 집합도 z마다 달라지는데**, 결품을 각 z의 자기 후보 집합에서 잰다
(`bc.stockout(net, candidates, ...)`). 분모가 흔들려 재배치 **전** 값이 z에 따라
2.0275 → 1.7798로 움직인다. 상한 격자에서 같은 결함이 결론을 뒤집었다
(docs/분석/EXPERIMENTS.md 17장). 고정 모집단으로 다시 재는 것은 **TODO 대기-10**.
