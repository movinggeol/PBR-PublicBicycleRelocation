...
🔴 **이 스크립트의 절대값을 쓰지 마라 (1.26.64).** `z`가 `target_qty`를 바꾸므로
**후보 집합도 z마다 달라지는데**, 결품을 각 z의 자기 후보 집합에서 잰다
(`bc.stockout(net, candidates, ...)`). 분모가 흔들려 재배치 **전** 값이 z에 따라
2.0275 → 1.7798로 움직인다.

    → 대신 experiments/params/z_fixedpop_grid.py 를 쓴다 (모집단 셋으로 잰다)

✅ **판정 자체는 다시 재도 같다** — 중립 모집단(전체 대여소)에서 z 격자 전체 폭이
5.2초(0.075%)라 가릴 수 없다(docs/분석/EXPERIMENTS.md 18장).
