...
`demand_model.FEATURES`에는 그 대여소의 지난달 통계(`prev_*` 9개)와 달·시간대·
요일 구분·계절 배율·날씨(도시 전체 값)가 있지만, **거치대 수·좌표 같은 대여소 고유
특성은 하나도 없다.**
...
def main() -> int:
    ...
    for duration in durations:
        for i in range(1, len(periods)):
            ...
            # `_10_15`(작업 대상 중앙값 33곳)에서 9달 중 4달만 남았고, 그 4달이
            # 모두 '빌리기가 이긴 달'이라 승률 100%(4/4)로 찍혔다. 9달을 모두
            # 넣으면 7/9(78%)다 — 탈락한 5달은 3승 2패였다. 통계가 성립하는
            # 최소선만 남긴다.
            if work.sum() < 10:
                continue
