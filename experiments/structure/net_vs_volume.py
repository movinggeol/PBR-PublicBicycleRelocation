...
이용량이 아니라 **순수요(대여 − 반납, 양수면 자전거가 빠져나가 채워야 하는 곳)** 다. 비가 와서 대여와 반납이 함께 반토막
...
- 같이 움직인다 → 날씨가 이용량을 통해 순수요까지 전달될 **경로는 있다**(필요조건).
  실제로 붙일지는 weather_impact.py의 표본 밖 판정으로 정한다.
...
def main() -> int:
    with db.session() as conn:
        ...
        # 재배치 필요량(Σ|순수요|) 변동 중 평일/휴일 구분이 설명하는 몫.
        # 나머지가 날씨 등에 남은 여지다.
