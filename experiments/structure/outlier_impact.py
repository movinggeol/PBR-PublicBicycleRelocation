...
**말로는 안 끝납니다.** 그래서 잽니다.
    ...
    ③ **계획이 달라지는가**          ← 여기서 달라지면 어느 쪽이 옳은지 이어서 잰다
    ④ IQR이 자르는 것이 정말 오류인가
    ⑤ 명백한 오류만 거르면 어떻게 되는가
    ⑥ 순수요가 실제로 쓰는 네 필드는 성한가
...
# 다르다 — EDA(concat_1year_file.preprocessing)는 1년치 병합 파일에서 컬럼을 순차로
# 자르고, 여기서는 한 달치에서 두 컬럼을 독립으로 재 OR로 합친다. 그래서 걸리는 행
# 수가 EDA 산출물과 정확히 일치하지는 않는다.
