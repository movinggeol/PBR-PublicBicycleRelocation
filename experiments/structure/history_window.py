...
운영은 `--period` 한 달(기본값은 적재된 **가장 최근 달**, `latest_period()`)로
대여소별 mu·sigma를 내고 계절 보정(warmup 14일)으로 수준을 맞춘다. 이 실험의
베이스라인 `prev1`은 그 한 달이 계획 달의 **바로 앞 달**인 경우를 가정한다. raw_data에는 12개월이 있는데 한 달만 쓴다.
...
def year_ago_window(periods, test_period, spread=1):
    ...
    **왜 ±로 넓히는가**: 작년 같은 달 하나면 표본이 대여소당 평일 17~23일(달별 중앙값)뿐이고,
...
def warmup_scale(stats, test, days=14):
    ...
    도시 전체 하나의 배율을 0.5~2.0으로 자르는 것은 운영 코드
    (`demand_model.season_ratio`)와 같다.

    🔴 **다만 한 가지가 다르다** — 운영은 배율을 구할 때 `|mu| > 2`인 대여소만
    쓴다(`WARMUP_MIN_DEMAND`). 0 근처 대여소를 넣으면 `E|X̂| > |E X|` 때문에 배율이
    과대추정되기 때문이다. 이 실험은 그 필터 없이 전체로 재므로, 여기서 나온 배율은
    운영보다 조금 크게 잡힐 수 있다. 학습 창끼리의 **상대 비교**가 목적이라 그대로
    두지만, 절댓값을 운영 값으로 인용하지 말 것.
    """
