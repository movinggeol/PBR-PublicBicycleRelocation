...
파이프라인은 1.14.0 전까지 평일만 다뤘다(raw_to_net의 `평일유무` 필터). 주말을
넣기 전에 **평일과 주말을 한 통계로 묶어도 되는가**를 이 스크립트로 쟀고, *묶으면
안 된다*는 결론에 따라 `day_type`이 실행 설정(`--day-type`)으로 올라갔다(1.14.0).

⚠️ **여기서 '평일'은 월~금(공휴일 포함), '주말'은 토·일이다.** 저장소 규약의
휴일(주말 ∪ 공휴일)과 다르다 — 공휴일을 가르는 것은 1.14.1의 `holiday_impact.py`다.
