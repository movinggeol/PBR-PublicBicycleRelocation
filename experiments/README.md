...
| `observed_stockout.py` | 결품을 관측에서 직접 세면 복원과 얼마나 다른가 | 🔴 **하루치 +13% → 4일치 +7% → 20-B −26% → 32장 −10%의 흔들림은 상당 부분 하네스 결함이었다** — 복원 기준이 요일·실행 종류를 안 가렸고(1.26.270), 모집단은 `MAX(run_label)`로 평일 γ 실험의 대상을 집었다(1.26.274). 고쳐 **실행별로 짝지어** 재면 여덟 칸 가운데 일곱 칸에서 실측이 크다(평일 +12·+21·+46·+92% · 휴일 +42·+70·+79%). **휴일 05~10시만 −35%** 인데, 같은 234곳을 평일로 재면 1.72h·휴일로 재면 1.21h이므로 **날 탓**이다. 편향이 **셋**(0 절단 · 공사의 실제 재배치 · 지난달 순수요의 추정 오차)이라 '하한'으로도 '과대추정'으로도 단정하면 안 된다 |
