...
### 지도 범례 시안을 실제로 비교한 결과 (1.26.79)

시안을 만든 쪽(1.26.75)은 *"채택 여부는 비교해 본 뒤 정한다"*로 남겨 뒀습니다.
`2026-08-28 도로실측2 _15_20` 스냅샷으로 여섯 장(시안 3·기존 3)을 실제로 띄워
재 본 결과는 **셋이 서로 다릅니다** — 통째로 채택하지 않습니다.

| 지도 | 판정 | 근거(실측) |
| --- | --- | --- |
| step4 재고 현황 | **채택할 만하다** | 범례가 156px로 짧고, 영어 `Legend`·회색 2px 테두리가 나머지 화면과 갈리던 것이 사라집니다 |
| step1 군집 | **보류** | 범례가 **610px**입니다. 18개 군집을 한 줄씩 세우면 세로가 화면의 2/3를 먹습니다 — 원래 범례가 없던 화면에 이만한 상자를 들이는 값어치는 따로 따져야 합니다 |
| step3 경로 | **보류** | 같은 이유로 **688px**. 기존 범례는 199px였고, 늘어난 489px 전부가 "군집 N 경로" 줄입니다 |

군집이 8개를 넘으면 색만으로는 어차피 다 구분되지 않습니다(팔레트가 8색).
**레이어 컨트롤이 이미 군집 이름을 한 줄씩 보여 주고 껐다 켤 수 있으므로**,
군집 목록을 범례에 한 번 더 세우는 것은 같은 정보를 두 벌 놓는 쪽에
가깝습니다. 채택한다면 군집 줄은 접어 두거나 빼고, **범례를 한 벌로 모으는
것(`legend_html`)과 재고 현황 지도의 한글화**만 가져오는 편이 낫습니다.

비교 중 시안에서 결함 둘을 찾아 고쳤습니다(둘 다 `tests/test_mapviz_shared.py`가 지킵니다):

- **군집 7과 15가 똑같은 검정이었다** — 8색을 순환하며 어둡게만 만드는데
  검정(`#000000`)은 어두워지지 않습니다(실측: RGB 거리 0). 어두운 색은
  밝히는 쪽으로 돌리게 고쳐 18개 군집이 모두 다른 색이 됐습니다.
- **경로 지도 범례에서 `▶ 차량 이동 방향`이 빠져 있었다** — 지도에는
  `PolyLineTextPath`로 화살표를 실제로 그리는데 범례가 그 기호를 설명하지
  않았습니다(기존 step3 범례에는 있던 줄입니다).
