...
| `broken_collect.py` | **고장 추정 자전거를 언제 수거하나** · 재배치에 끼워 넣을 수 있나 | **좌표를 지어낼 필요가 없었다** — `bike_no`로 745대를 관측에서 추론했다(위치 = 마지막 반납 대여소). 소멸률이 이용량에 **단조 감소**(27.3% → 8.5%)라 마모설이 기각되고 *“상태가 나빠 안 빌려간다”* 가 지지된다. **무릎은 적재 용량(10대)**, **대수 기준이 일수 기준을 지배**하며, 재배치와 **26% 겹친다**(34장). 세대 층화(F, `--audit-only`의 감사 ③)는 사전 등록 판정이 **DJ3에서만 지지**다 — DJ3는 세대 안에서도 엄격히 단조, DJ2는 두 분위 역전(+0.6%p·+0.1%p)으로 기준 미달 |
