...
| `budget_enforce.py` | 시간 예산을 제약으로 걸면 무엇을 잃나 | ⚠️ **1.26.281 전의 결품 값은 출발 재고가 다른 실행의 것이었다**(5-E 정정 — 정본으로 다시 재면 켠 채 세 회차 모두 나빠짐). 추정 120분 기준으로는 결품 +40초로 싸 보이지만, **그것은 실도로로 158분을 허용한 것**이었다(5-E장). 실제 120분을 지키면 물량의 약 16%(126대)가 미집행되고 결품 악화는 3.6배. 기본은 계속 꺼 둠 |
...
| `observed_stockout.py` | 결품을 관측에서 직접 세면 복원과 얼마나 다른가 | 🔴 **하루치 +13% → 4일치 +7% → 20-B −26% → 32장 −10%의 흔들림은 상당 부분 하네스 결함이었다** — 복원 기준이 요일·실행 종류를 안 가렸고(1.26.270), 모집단은 `MAX(run_label)`로 평일 γ 실험의 대상을 집었다(1.26.274). 고쳐 **실행별로 짝지어** 재면 여덟 칸 가운데 일곱 칸에서 실측이 크다(평일 +12·+21·+46·+92% · 휴일 +42·+70·+79%). **휴일 05~10시만 −35%** 인데, 같은 234곳을 평일로 재면 1.72h·휴일로 재면 1.21h이므로 관측 쪽은 **날 탓**이다. 🔴 **복원 쪽은 원인을 못 갈랐다** — 복원이 출발한 오후 스냅샷이 실제 회차 시작보다 3.9~23.7%p 더 비어 있어 여덟 칸 모두 위로 기울어 있다(2026-09-23). 몫이 넷(0 절단 · 공사의 실제 재배치 · 출발 재고 · 지난달 순수요)이라 '하한'으로도 '과대추정'으로도 단정하면 안 된다 · `--same-day`로 출발 재고를 맞춘 비교를 한다(1.26.278) |
...
| `dockless_hub.py` | **도크리스로 가면 재배치의 단위는 무엇이 되나** — 어디를 거점(가상대여소)으로 삼나 | **기준이 갈랐다 (36장).** 1차(이용량): 상위 543곳이 끝점 80%를 덮는 데 전 지점의 38.6%가 들고(①미달), 그 거점이 정본 후보의 **64.7~73.9%** 만 포함해(②미달) *'많이 쓰는 곳'은 '옮겨야 할 곳'이 아니다* — 후보 이용량 순위 중앙값 318~386위/1,408곳. 2차(`--criterion demand`, 회차별 \|평균 순수요\|의 최댓값): 같은 543곳이 후보의 **70.7~85.8%** 를 덮고(①통과) 정본 달을 뺀 표본 밖에서도 71.3~85.5%(①-b통과), 끝점도 70.1% 덮는다(④통과). 🔴 그러나 거점 집합의 달 사이 자카드가 0.78~0.92 → **0.49~0.72**로 나빠진다(③미달) — 편향은 두 큰 수의 차라 한 달 표본에서 잡음이 크고, 반년씩 묶으면 0.705(이용량 0.860). ▶ 설계는 두 층: 거점 위치는 **여러 달 묶음**으로, 회차 물량은 지금처럼. 🔴 좌표가 **대여소 좌표**라 전부 **상한**이다. ⏸ 후속 구현(귀속 반경 모사 등)은 **범위 밖**(2026-09-16 사용자 결정) — 논문에는 방법의 일반성으로만 쓴다 |
...
| `stage_timing.py` | 3단계 분해가 정말 '풀리는 크기'를 만드나 — 군집화·ILP·경로를 나누어 잰다 | **군집화 2.2~2.8초 · ILP 군집당 0.03초 · 경로 1ms 미만**(환경 A). 원고가 적고 있던 *42초 · 군집당 수 초* 는 1.23.8 이전의 조건 없는 값이었다(37장) |
