...
| `broken_collect.py` | **고장 추정 자전거를 언제 수거하나** · 재배치에 끼워 넣을 수 있나 | **좌표를 지어낼 필요가 없었다** — `bike_no`로 745대를 관측에서 추론했다(위치 = 마지막 반납 대여소). 소멸률이 이용량에 **단조 감소**(27.3% → 8.5%)라 마모설이 기각되고 *“상태가 나빠 안 빌려간다”* 가 지지된다. **무릎은 적재 용량(10대)**, **대수 기준이 일수 기준을 지배**하며, 재배치와 **약 4분의 1** 겹친다(34장 — 2회차 26.0%, 낮 세 회차로 다시 재도 24.7~25.8%라 사전 등록 판정은 **곁가지**, 로드맵 D). 세대 층화(F, `--audit-only`의 감사 ③)는 사전 등록 판정이 **DJ3에서만 지지**다 — DJ3는 세대 안에서도 엄격히 단조, DJ2는 두 분위 역전(+0.6%p·+0.1%p)으로 기준 미달. 🔴 우측 절단(E, `--verify`)은 창을 앞으로 옮겨 재니 소멸 판정의 **41.8%가 반년 안에 돌아와** 사전 등록 판정이 *'34장의 대수를 다시 써야 한다'* 다. 양성 대조(G, `--positive-control`)는 전 기간 129대 중 **97.7%가 90일 안에 복귀**해(대조 99.8%) 월별 방향 조건에서 **판정 못 함**이지만, 관측은 *정비대기 반납 = 수리 뒤 복귀* 쪽이다. 야간까지 넣은 하루 전체(D-6)도 **31.3~31.9%**로 곁가지이고 곡선이 안 꺾인다. ⏸ **2026-09-15부터 보류**([로드맵](../docs/기록/고장수거_로드맵.md) 맨 위) |
| `dockless_hub.py` | **도크리스로 가면 재배치의 단위는 무엇이 되나** — 대여·반납이 잦은 곳을 거점(가상대여소)으로 삼을 수 있나 | 판정 장치만 커밋했다 (2026-09-16) — 사전 등록: ① 상위 K ≤ 전 지점의 30%로 통행 끝점 80% 커버 ② 정본 후보 포함률 ≥ 70%(세 회차) ③ 달 사이 자카드 ≥ 0.70. 🔴 좌표가 **대여소 좌표**라 이 커버리지는 **상한**이다(도크리스의 실제 분산이 아니다). 계기는 중기계획 3-5-1(도크리스 QR 2,500대)과 부록 자문의견 *"가상대여소의 위치 선정에 대한 분석이 필요"* |
...
python experiments/structure/dockless_hub.py       # 도크리스 거점(가상대여소) 후보 진단
```
