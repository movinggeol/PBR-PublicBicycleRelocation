...
DEFAULT_RAW_FILE = os.getenv(
    "PBR_RAW_FILE",
    "data/raw_data/타슈 대여이력(25.04~26.03).csv",
)
# ⚠️ 이 파일은 25년 04월~26년 03월 12개월이 **한 파일에 섞여 있다**(1.26.187).
# `raw_to_net.py`는 이 파일을 통째로 읽어 config.period 이름으로 저장하므로,
# **이 파일이 실제로 읽히는 것은 그 기간이 DB(rental_history)에 아직 없을 때뿐이다**
# — 그때는 12개월치가 그 한 기간으로 잘못 라벨링된다. 지금은 12개월 전부
# 이미 DB에 있어(webapp store.periods_with_rentals()) `/run` 폼이 이 칸을
# 흐리고 쓰지 않는다(1.26.184) — 그래서 이 위험이 실제로는 드러나지 않는다.
# 새 기간(이 범위 밖)을 다룰 때는 그 기간만 담긴 단일 CSV로 반드시 바꿔라.
...
# 차량 최대 적재 대수. **상한이지 통상값이 아니다** — 관제센터 유선 문의
# (2026-05-15)에서 "통상 7대, 많이 실어야 10대"라 답했다(docs/기록/ORIGINS.md 4장).
VEHICLE_CAPACITY = 10
...
# **대전교통공사 타슈 관제센터 유선 문의로 확인한 값이다** — 통화는 **2025-09-06**,
# 저장소 기록은 1.26.57(2026-08-31)이다. 두 날짜를 혼동하지 마라: 논문 각주에 쓸
# 출처 일자는 **통화한 2025-09-06**이다(docs/기록/ORIGINS.md 4장).
# 총 21대를 보유하되 한 번에 전부 투입하지 않고 조를 나눠 운용한다 — 이 코드의
# 배분 모델과 정확히 일치한다. 회차당 실제 소요는 12~16대다(근거는 docs/구현/FLEET.md).
# 21대는 **넘을 수 없는 상한**이므로 그 위로 올려서 재지 않는다.
# ⚠️ 담당자 **구두 진술**이고 공개 자료에는 차량 대수가 없다. 검색하면 나오는
# "21명"은 수거 **인력**이라 다른 값이다 — 논문 각주에서 갈라 둘 것.
# `--fleet-size`로 실행마다 바꾼다.
...
#
# 🔴 **이 값은 운영 주체의 제약이 아니라 연구자가 세운 목표다**(사용자 확인,
# 2026-09-03). "1~2시간 이내 완료" 메모에서 왔다. FLEET_SIZE(21대)·VEHICLE_CAPACITY
...
#
#   추정 총 소요 = 처리 대수 x (수거 + 배송)        <- 정확히 계산된다
