...
# 차량 **순항** 속도(km/h). **ILP와 VRP가 반드시 같은 값을 써야 한다** —
# 두 단계가 다른 속도로 계산하면 ILP가 최소 비용이라고 고른 조합이 VRP에서는
# 최소가 아니게 된다. 1.13.2에서 25로 통일(VRP가 30이었음).
#
# **1.23.0에서 실측했다** — TMAP 도로 소요 3,051구간의 회귀 기울기가 25.4 km/h로
# 나와, 순항 속도로서는 25가 맞았다. 틀렸던 것은 아래 TRAVEL_STOP_SEC이 없었던 것이다.
VEHICLE_SPEED_KMPH = float(os.getenv("PBR_VEHICLE_SPEED_KMPH", "25"))

# 구간마다 거리와 **무관하게** 드는 시간(초) — 출발·정지·신호·진입.
#
# ⚠️ **이 값이 없으면 이동시간이 구조적으로 과소 추정된다.** 실측(1.23.0):
# 구간 속도가 0.5km 미만에서 7.7 km/h, 8km 이상에서 24.0 km/h로 **거리에 따라
# 변한다**. 고정 속도로는 담을 수 없는 구조이고, 원인은 짧은 구간일수록 정차
# 비용의 비중이 크기 때문이다. TMAP 실측 회귀의 절편이 2.39분이었다.
#
# 이걸 넣기 전에는 회차당 이동시간이 15분(80 → 95분) 낙관적이었고, 시간 예산
# 초과가 7%로 보였다(실제 13%). 근거·재현: experiments/structure/travel_time_model.py
TRAVEL_STOP_SEC = float(os.getenv("PBR_TRAVEL_STOP_SEC", "143"))


def travel_seconds(km: float) -> float:
    """거리(km) → 이동시간(초). **ILP·VRP·시간예산이 모두 이 함수를 쓴다.**

    `정차 비용 + 거리 / 순항속도` 의 아핀 모델이다. 속도 상수 하나만 쓰면
    짧은 구간을 심하게 과소 추정한다(실측에서 0.5km 구간이 7.7 km/h).

    **함수 하나로 모은 이유**: 예전에 ILP가 25, VRP가 30을 쓰다가 두 단계가
    어긋난 적이 있다(1.13.2). 상수를 공유하는 것만으로는 계산식이 갈리는 것을
    막지 못하므로, 계산 자체를 여기 한 곳에 둔다.
    """
    return TRAVEL_STOP_SEC + (float(km) / VEHICLE_SPEED_KMPH) * 3600.0
