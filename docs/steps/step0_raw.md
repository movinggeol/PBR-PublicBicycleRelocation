...
- **처리**: `--day-type`(평일/주말) 필터 → 대여/반납 건수 집계 → 주차대수·재고 병합
  ...
  1.14.0 이전에는 평일 고정이라 **주말에만 쓰이는 대여소가 통째로 빠졌다**)
...
- **처리**: **평일·주말 모두**, 날짜×대여소×시간대(0~23시)별 `순수요 = 대여량 − 반납량`
  (어느 쪽으로 계획할지는 다음 단계가 `--day-type`으로 고른다)
- **출력**: `data/pp_data/순수요/st_net_daily ({period}).csv` (net_00 ~ net_23 컬럼)

## 평일과 주말은 섞지 않는다 (1.14.0)

첫 커밋부터 파이프라인은 **평일만** 다뤘습니다(`raw_to_net`의 `평일유무` 필터).
주말 이용이 전체의 **21~31%**인데 통째로 버려지고 있었고, 주말 계획은 아예
만들 수 없었습니다.

이제 `raw_to_net`이 두 요일을 모두 계산하고, **`calculate_target_qty`가 한쪽만
골라** 통계를 냅니다(`--day-type weekday|weekend`, 기본 weekday).

**왜 골라야 하는가 — 섞으면 상쇄됩니다.** 25년 11월 실데이터입니다.
...
>
> 공휴일은 아직 평일로 셉니다(토·일 기준). 반영하려면 외부 달력이 필요합니다.
