...
> ⚠️ **5번에서 끝나면 그 자리에서 멈춘다 — 마지막 depot 복귀가 없다.**
> 설계는 "차량 1대 = 클러스터 1개 + depot 복귀"인데(FLEET.md) 계산에는 복귀 구간이
> 빠져 있어, **이동거리·소요시간·시간 예산 판정이 그만큼 낙관적**이다.
> 4번이 복귀를 기록하는 유일한 자리인데 실행되지 않으므로, **가끔이 아니라 항상 빠진다.**
> 실측: `_05_10` 한 회차에서 84.2km 누락, 예산 초과 2건 → 복귀 포함 시 5건.
> → [TODO.md](../TODO.md) 1-1
