...
토큰 기반으로 다시 만들었습니다. 색은 **역할로만** 쓰고, 라이트/다크 값은
`base.html`의 `:root` 블록 한 곳에서만 갈립니다.

| | |
| --- | --- |
| 배색 | 대비·색각 검증을 통과한 팔레트 기준. 표면/잉크/경계선/상태색을 역할 토큰으로 정의 |
| 다크 모드 | OS 설정(`prefers-color-scheme`) + 헤더의 토글. 토글이 양방향으로 OS를 이깁니다 |
| 상태 표시 | **색만으로 뜻을 전하지 않습니다** — 배지는 점+글자, 예산 초과 행은 붉은 배경 + `⚠` |
| 통계 타일 | 라벨·값·증감. 증감은 `▲`/`▼` 기호를 함께 붙입니다 |
| 표 | 헤더 고정, 숫자 열은 우측 정렬 + `tabular-nums`, 넓은 표는 자체 가로 스크롤 |
| 반응형 | 본문 1180px, 좁아지면 2단 → 1단 |

**다크 값은 세 곳에 선언합니다** — `:root`(라이트), `@media (prefers-color-scheme: dark)`
안의 `:root:not([data-theme="light"])`, 그리고 `:root[data-theme="dark"]`. 이렇게 해야
토글이 OS 설정을 양방향으로 이깁니다. 첫 페인트 전에 `<head>`의 인라인 스크립트가
저장된 테마를 적용해 새로고침 때 화면이 번쩍이지 않습니다.

현재 위치 표시는 라우트마다 변수를 넘기지 않고 `request.url.path`로 판단합니다.

> 클래스 이름이 **뜻**을 담게 했습니다. 증감은 `.delta.good` / `.delta.bad`이지
