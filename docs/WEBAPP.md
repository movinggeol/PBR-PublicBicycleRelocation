...
실행 화면의 '최신 산출물'은 분류 13개 × 2건 = 26행이라 옆의 실행 폼보다 훨씬
길었습니다. **분류 5개씩 쪽을 나눠** `‹ 이전` / `다음 ›`으로 넘깁니다.

행이 아니라 **분류 단위**로 끊습니다 — 분류 이름이 첫 행에만 붙어서, 행으로 자르면
이름 없는 행으로 시작하는 쪽이 생깁니다. 서버는 늘 전부 내려보내고 조작부는
스크립트가 만들므로, 스크립트가 없으면 예전처럼 전부 보입니다.
규칙 전문은 [DESIGN.md](DESIGN.md#쪽-넘기기-1174).

`data-pager="N"` + `data-page-item="키"` 두 속성만 붙이면 어느 표에서나 쓸 수
있습니다(`/data`·`/maps`에는 아직 붙이지 않았습니다).
