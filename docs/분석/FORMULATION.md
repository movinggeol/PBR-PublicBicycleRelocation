...
**운영 모델:** 하루 4회차(`_05_10`, `_10_15`, `_15_20`, `_20_05`), **차량 1대 = 군집 1개**,
회차 경계는 운영 관행이 아니라 이용의 큰 변화(출근 전·후, 퇴근 무렵·후)를 기준으로 연구자가
정했다 — 운영기관 문의에서 재배치에 정해진 시각은 없다는 답을 받았다(2025-09-06). 이 줄은
2026-09-16까지 *"하루 3회차"* 였다(초안 2장 각주 3이 짚은 잘못).
...
> 🔴 **실효 상한은 $Q$가 아니라 $Q-1$이다.** $\tanh$는 1에 점근할 뿐이라
> $|r_i| = Q$가 되려면 $|t_i - q_i| \ge 190$대여야 하는데(부동소수 $\tanh$가
> 1.0으로 포화하는 지점), 실산출물 10,740행의 최댓값은 **55.6대**이고 넘는
> 대여소는 **0곳**이다. 관측되는 $|r_i|$는 정확히 **9에서 끊긴다**
> (분포 `{4:25, 5:135, 6:129, 7:70, 8:66, 9:99}`).
>
> 그리고 이 완화는 상한만 자르는 것이 아니라 **전 구간을 아래로 당긴다**
> (5 → 4, 8 → 6, 10 → 7). 하드 클리핑으로 바꾸면 총 작업량이 **12.9% 늘어난다**
> (8,020행 실측: 10,260 → 11,584대). 바꾸지 않기로 한 근거는
> [DECISIONS.md 5-B](DECISIONS.md)에 있다.
>
> **이 수식을 인용할 때 $Q = 10$을 '한 대여소 최대 계획량'으로 읽으면 틀린다.**

> 코드: `calculate_target_qty.compute_rebal_qty()` — 저장할 때마다 그 실행의 실제 최댓값을 로그에 찍는다
...
> (단조 위반 0/9건, [EXPERIMENTS.md](EXPERIMENTS.md) 17장). 그런데 **비용이
> 같이 늘어 집행이 막힌다** — 상한 100에서는 12대 중 **11대가 시간 예산을
> 넘긴다**. 결품과 예산 준수가 정면으로 맞바뀌므로, 초과를 몇 대까지
> 받아들일지가 정해져야 $N$이 정해진다. **운영 정책의 선택**이다 —
> [DECISIONS.md 6-C](DECISIONS.md#6-c-후보-상한-50--내리지-않는다-내리자던-근거가-잘못된-자였다-2026-09-01-전면-개정)에
> 적었다.
>
> ⚠️ **12장의 상한 표를 인용하지 말 것** — 상한마다 다른 모집단에서 잰 값이라
> 무효다(17장에서 정정).
...
> 📌 **2026-09-17 — 위 계수는 곧 바뀐다.** 고정 패널 10일로 사전 등록 판정을 통과한 값은
> $F$=**320.4초**, $v_r$=**32.11 km/h**다(2026-09-15, [EXPERIMENTS.md](EXPERIMENTS.md) 9장 *판정* 절 —
> 위의 275초·27.3 km/h는 하루치 한 번에서 나온 값이었다). 채택은 승인됐고(09-16) 09-20 켠 채 재실행은
> 이 값을 `.env`로 쓴다. 코드의 기본값이 이 값으로 바뀌는 것은 문서 재작성이 끝난 뒤다
> ([수집완료_계획.md](../기록/수집완료_계획.md) 2-4). 그때 이 절의 "기본값"도 실측 식으로 바꾼다.
