...
- [ ] 시간 예산을 **클러스터링(step1)** 제약으로 반영 — 기본은 사후 점검이다.
      `--enforce-time-budget`(`PBR_ENFORCE_TIME_BUDGET=1`, 1.21.6)을 주면 VRP가
      *'이 작업을 하고 depot으로 돌아오면 예산을 넘는'* 작업 앞에서 멈추고 복귀한다
      (남은 대수는 미집행 — `experiments/baseline/budget_enforce.py`)
