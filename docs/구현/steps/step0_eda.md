# Step 0 — 전처리 및 EDA (`pipeline/step0_eda/`)
...
> 같은 파일에 반복 적용하면 계속 깎입니다(50만 행 표본 실측 — 각 회차가 **직전
> 회차 결과 대비** 남기는 비율: 1회 94.1% → 2회 96.4% → 3회 96.0% → 4회 97.6%,
> 누적하면 **네 번에 원본의 15%가 깎입니다**). 그런데 이 단계는
