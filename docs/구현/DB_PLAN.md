...
> **배선은 step0~step4 전부 DB로 옮겼습니다**(1.26.164·166·167). CSV를 0장으로
> 만들어도 `run_pipeline --skip-api`가 8단계를 완주합니다. 남은 것은 `to_csv`
> 제거뿐인데 **사용자 지시로 보류합니다** — 아래 5단계 절 참고.
...
이것이 **컬럼 수 민감도**를 설명합니다 — 아래 수치는 방금 본 150,000행×12컬럼
분해표(0.564초/0.244초)가 아니라, 앞서 나온 **594,000행 시나리오**(4·9컬럼,
"성능 — 측정해 보니 예상과 달랐습니다" 절의 표)에 12컬럼(원본 `rental_history`
폭 전체)까지 같은 규모로 추가 측정한 결과입니다. DB 비용은 만들어야 할 셀
개수에 정비례하고(4→0.98초, 9→1.79초, 12→2.23초), CSV는 바이트를 훑는 비용이
지배적이라 완만합니다(0.89 → 0.99 → 1.14초).
...
| ④+ | step3 지도·`--skip-api` 가드도 DB로 | ✅ 완료 (1.26.167) |
...
### 🔴 "다 옮겼다"고 했지만 두 곳이 남아 있었다 (1.26.167)

*"CSV 없이 DB만으로도 도는가"* 라는 물음에 답하려고 **실제로 CSV를 전부 지우고
완주시켜 봤더니** 두 곳이 걸렸습니다. 그 전까지는 단계별 읽기 함수만 확인했지,
지우고 돌려 본 적이 없었습니다.

| 어디 | 무엇이 문제였나 |
| --- | --- |
| `step3_map/main.py` | **파일만** 봐서, DB에 자료가 있어도 CSV가 없으면 경로 지도가 한 장도 안 나왔다(조용히 건너뛴다) |
| `run_pipeline.inherit_snapshot()` | 스냅샷을 **파일로만** 찾아, `--skip-api`가 *"물려받을 스냅샷이 없습니다"* 로 **단계가 시작되기도 전에** 멈췄다 |

둘 다 DB를 함께 보게 고쳤습니다. 이제 산출물 CSV를 0장으로 만든 뒤
`run_pipeline --skip-api`를 돌리면 **8단계가 전부 실행되고** 경로 지도까지
그려집니다(건너뛴 시간대 0개).

`tests/test_pipeline.py`가 이 시험을 그대로 자동화합니다 — 한 번 완주시켜
CSV를 만든 뒤 **전부 지우고 다시 돌려**, *"입력이 없습니다"* 가 한 줄도 없어야
통과합니다.
