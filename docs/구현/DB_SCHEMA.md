...
넘어야 하지만, 지난달 순수요로 이번 달을 맞히는 **추정 오차**가 반대로 걸리면
작아집니다 — 그 회차의 작업 대상을 휴일 이틀로 재면 결품 1.21시간에 만차
1.22시간이고, 같은 대여소를 평일 엿새로 재면 결품 1.72시간입니다(휴일 새벽에는
자전거가 밤새 나가지 않습니다). 편향이 둘이 아니라 셋이라는 뜻입니다
([EXPERIMENTS.md](../분석/EXPERIMENTS.md) 32장).
