...
> `run_label`은 **실행 시각이 아니라 라벨**입니다. `2026-05-21 18` 같은 문자열입니다.
>
> 🔴 **"문자열 정렬이 곧 최신순"은 규약이었지만 지켜지지 않습니다** (1.26.125에
> 실측). 실험은 `obs-cmp-1520` 처럼 이름을 붙이는데 `'o' > '2'`라 **어떤 날짜
> 라벨도 이깁니다.** 그래서 실험을 한 번 돌리면 그 뒤로 계획을 아무리 돌려도
> `MAX(run_label)`은 영영 실험을 가리켰습니다 — 여섯 테이블이 그 상태였고,
> 라벨 없이 연 `/orders`가 **실험을 현장 지시서로** 내고 있었습니다.
>
> 지금은 **`runs.created_at`으로 고릅니다**(`db.latest_label()`). 정렬을 라벨
> 모양에 기대지 마세요 — 라벨은 사람이 붙이는 이름입니다.
...
| `kind` | TEXT | `plan` / `experiment` / `probe` (`db.RUN_KINDS`). 아래 참고 |
...
🔴 **`kind`는 화면이 무엇을 '마지막 계획'으로 부를지 정합니다.** 실험도 같은
테이블에 쌓이므로, 종류를 모르면 가장 최근 실행이 실험일 수 있습니다 — 실제로
목록에 없던 `obs-cmp-1520`이 경고 없이 첫 화면 헤드라인에 올라왔습니다
(1.26.107). 그때 넣은 컬럼입니다.

**누가 아는가가 핵심입니다.** 실험도 같은 파이프라인으로 돌기 때문에 step
스크립트는 **원리상** 자기가 실험인지 알 수 없고, 아는 것은 띄우는 쪽뿐입니다.
그래서 `run_pipeline --run-kind`가 `PBR_RUN_KIND`로 내려보내고, 웹 실행 폼은
언제나 `plan`을 붙입니다(1.26.114).

⚠️ **`NULL`이면 `classify_run_label()`이 라벨로 짐작합니다.** 기본값을 `plan`으로
두지 **않은** 것이 핵심입니다 — 선언을 잊은 실험이 계획으로 확정되면 짐작보다
나쁩니다. 짐작은 없애지 않고 **그물로 내렸습니다.** 채우는 자리는 한 곳
(`db.list_runs()`)이라, 화면이 종류를 다시 짐작하지 않습니다. 사람이 고치려면
웹의 실행 목록에서 종류를 바꿉니다(`POST /runs/{run_label}/kind`).
...
3. **마스터를 `station_stock`에 얹지 않았습니다.** 당시 `latest_label()`이
   ...
   항상 커서 파이프라인 실행을 밀어냈습니다. (1.26.125에서 `runs.created_at`
   기준으로 바꿨지만 **이 결정은 그대로 둡니다** — 수집은 파이프라인 실행이
   아니므로 `runs`에 줄을 남길 일이 아니고, 아래 4번 이유도 여전합니다.) `runs` 이력도
...
### 4-8. 실측 관측 — 파이프라인이 만들지 않은 정답표

둘 다 **추정치를 검증하려고** 둔 표입니다. 파이프라인 산출물이 아니라 바깥에서
들어온 값이라, 이것이 없으면 "우리 가정이 맞나"를 물을 방법이 없습니다.

**`road_leg`** — TMAP이 준 **실제 도로 소요시간** (1.23.2, 스코프: `run_label` + `duration`)

step3이 지도를 그리며 받은 값을 그대로 남깁니다. `VEHICLE_SPEED_KMPH = 25`가
맞는지 검증할 유일한 근거입니다.
매일 같은 구간을 다시 재는 고정 패널분(`roadprobe-*`, [COLLECTOR_ROAD.md](COLLECTOR_ROAD.md))은
게이트 A 판정(2026-09-15 통과)의 표본입니다.

🔴 **저장 규칙이 둘입니다 (1.26.232).** 회차의 모든 군집을 TMAP으로 받았으면 `save_output`이
(실행, 회차) 범위를 통째로 갈아끼웁니다. **호출 도중 한도가 끊겨 일부만 받았으면**
`db.replace_road_legs()`가 **받은 군집의 행만** 갈아끼웁니다 — 범위째 갈았더니 못 받은 군집의
옛 실측이 사라져 한 회차가 80행에서 41행이 됐습니다. 실측은 같은 시각의 교통량으로 다시 잴 수 없습니다.

| 컬럼 | 타입 | 설명 |
| --- | --- | --- |
| `run_label`, `duration` | TEXT | 스코프 |
| `cluster`, `leg` | INTEGER | 어느 군집의 몇 번째 구간인가 |
| `from_id`, `to_id` | TEXT | 구간 양끝 대여소 |
| `from_lat`/`from_lon`, `to_lat`/`to_lon` | REAL | 좌표 |
| `straight_km` | REAL | 직선(Haversine) 거리 — 파이프라인이 쓰는 값 |
| `road_sec` | REAL | **TMAP 실측** 소요(초) |
...
step4의 결품은 순수요로 **복원**한 값이라 재고 0에서 잘려 낮춰 잡습니다.
관측(`stock_history`)과 맞대어 그 격차를 계수로 남깁니다.

| 컬럼 | 타입 | 설명 |
| --- | --- | --- |
| `measured_at` | TEXT **PK** | 계수를 잰 시각 |
| `duration`, `day_type` | TEXT **PK** | 회차 / `weekday`·`holiday` |
| `ratio` | REAL | 실측 ÷ 복원. **1보다 크면 복원이 낮춰 잡은 것** |
| `observed`, `simulated` | REAL | 관측 결품(h) / 복원 결품(h) |
| `days`, `stations` | INTEGER | 근거가 된 온전한 날 수 · 대여소 수 |
| `note` | TEXT | 관측 창과 **짝지은 계획 실행 수**(1.26.274) — 예: `관측 창 00~23시 · 계획 실행 2건`. 계수가 실행 한 건에 기댄 값인지 여럿의 평균인지가 읽는 데 필요합니다 |

**이 표가 있는 이유가 곧 설계의 요지입니다.** 수집을 멈추면 관측은 사라지지만
계수는 남습니다 — 언제 무엇으로 잰 보정인지를 함께 적어 두므로, 나중에도
*"며칠치 관측으로 얻은 계수인가"* 를 밝히며 인용할 수 있습니다.
관측이 없다고 지표가 무너지지 않게 하는 것이 목적입니다
([KPI.md](../분석/KPI.md)).

🔴 **`ratio`가 1보다 작은 칸이 있습니다 — `ratio > 1`을 전제하는 코드를 쓰지
마십시오.** 2026-09-22 휴일 `_05_10`이 **0.646**입니다. 복원의 0 절단만 보면 1을
넘어야 하지만 다른 몫이 반대로 걸리면 작아집니다. 관측 쪽은 **날 탓**으로 재 두었고
(같은 대여소가 휴일 이틀 1.21시간 · 평일 엿새 1.72시간), 복원 쪽은 **출발 재고**가
실제보다 더 비어 있어(오후 스냅샷 45.7% · 실제 휴일 05:00 22.0%) 위로 기울어 있습니다.
어느 몫이 얼마인지는 아직 가르지 못했습니다([EXPERIMENTS.md](../분석/EXPERIMENTS.md) 32장).

⚠️ **`ratio`는 계획 시각의 재고에서 출발한 복원을 분모로 씁니다.** 계획을 오후에 세우면
새벽 회차의 분모가 부풀어 `ratio`가 작게 나옵니다 — 계수를 인용할 때 그 실행의
`runs.created_at`을 함께 보십시오.

⚠️ **`ratio`는 실행별로 짝지어 잰 값의 평균입니다**(1.26.274). 복원값을 낸 계획
실행마다 **그 실행의 작업 대상**으로 관측을 재고 나서 평균합니다 — 복원이 실행
여럿의 평균인데 모집단이 집합 하나면 자가 어긋나기 때문입니다.
