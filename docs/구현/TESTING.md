...
파이프라인이 아예 돌지 않았습니다. 그때 만든 안전망이 지금의 768개 테스트입니다.
이후 모든 수정은 이 위에서 이뤄졌습니다.

```powershell
python -m pytest              # 전체 768개 (약 200초)
...
| [tests/test_experiment_guards.py](../../tests/test_experiment_guards.py) | 25 | **실험 하네스가 조용히 틀린 답을 내지 않는지** — 없는 라벨을 받으면 작업량 0인 표를 말없이 내놓던 것(1.26.39), 라벨이 없을 때 *"다시 돌리세요"* 라는 **틀린 처방**을 안내하던 것(1.26.54), 그리고 **실측 결품 보고서가 어느 날을 셌다고 말하는지**(1.26.123) — 날 판정은 **그날의 창**으로 재 자료를 더해도 안 움직이고(1.26.153, 예전엔 가장 넓은 날이 잣대라 좁은 날이 소급 탈락했다), 회차 판정은 그 시간대만 본다 · 창이 회차를 못 덮으면 밝히는지 · 반쪽짜리 날이 섞이면 관측시간과 결품시간이 함께 줄어드는지, 그리고 **어느 요일 구분으로 채점했는지**(1.26.127) — 실험은 `auto`(**돌린 날의 달력**)를 쓰고 있어 일요일에 평일 계획을 휴일 순수요로 재고 있었다 · 기록이 없으면 조용히 넘어가지 않는지. 그리고 **결품 예측이 스스로를 속이지 않는지**(2026-09-10, ML 아홉 번째 시도) — 🔴 **이겼을 때가 가장 위험하다**(7번 시도는 교차검증을 통과하고도 틀려 되돌려졌다): 날짜로 자르는지(무작위 분할은 같은 날 앞뒤 틱을 양쪽에 넣어 누출이 되고, 자기상관 0.790이면 그것만으로 이긴다) · 피처가 미래를 보지 않는지 · 과거 빈도표를 **학습 구간에서만** 만드는지(검증 값으로 만들면 누출) · Brier가 **순서가 아니라 확률의 크기**를 재는지(계획은 '어디부터 손댈지'에 확률을 쓴다). 여기서 나온 숫자가 논문에 실린다 |
...
| **합계** | **768** | 27개 파일 · 약 200초 (`python -m pytest`) |
