...
파이프라인이 아예 돌지 않았습니다. 그때 만든 안전망이 지금의 571개 테스트입니다.
이후 모든 수정은 이 위에서 이뤄졌습니다.

```powershell
python -m pytest              # 전체 571개 (약 100초)
...
| 파일 | 개수 | 무엇을 지키는가 |
| --- | --- | --- |
| [tests/test_webapp.py](../../tests/test_webapp.py) | 86 | 웹 라우트가 통째로 깨지는 사고 방지, 실행 폼 입력 검증(기간·시간대 목록 포함), 파일 목록 쪽 나눔, **표 정렬을 건 표/걸지 않은 표**, 표 열·행 접기와 그 상태 기억, 빈 상태 규약 ([WEBAPP.md](WEBAPP.md)) |
...
| **합계** | **571** | 25개 파일 · 약 120초 (`python -m pytest`) |
