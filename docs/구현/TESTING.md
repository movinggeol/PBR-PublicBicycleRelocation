...
파이프라인이 아예 돌지 않았습니다. 그때 만든 안전망이 지금의 745개 테스트입니다.
이후 모든 수정은 이 위에서 이뤄졌습니다.

```powershell
python -m pytest              # 전체 745개 (약 200초)
...
| [tests/test_pipeline.py](../../tests/test_pipeline.py) | 44 | step0→1→2→4를 **실제로 실행**해 산출물·스키마 확인, **도구가 실패를 종료 코드로 말하는지**(`redraw_maps.py`가 전부 깨져도 0으로 끝났다) |
...
| [tests/test_road_time.py](../../tests/test_road_time.py) | 26 | **TMAP 실도로 고정 패널** — 패널이 결정적인지(흔들리면 '매일 같은 구간'이 무너진다), 차고지 왕복을 포함하는지, 거리 구간이 고르게 차는지, 누적의 차분이 구간 실측이고 `start_time`이 함께 남는지, **한도가 소진돼도 늘 같은 회차만 잘리지 않는지**, **평일/휴일 자동 판정과 모형 재추정의 day_type 필터**(1.26.158) ([EXPERIMENTS.md](../분석/EXPERIMENTS.md) 9장) |
...
| [tests/test_experiment_guards.py](../../tests/test_experiment_guards.py) | 21 | **실험 하네스가 조용히 틀린 답을 내지 않는지** — 없는 라벨을 받으면 작업량 0인 표를 말없이 내놓던 것(1.26.39), 라벨이 없을 때 *"다시 돌리세요"* 라는 **틀린 처방**을 안내하던 것(1.26.54), 그리고 **실측 결품 보고서가 어느 날을 셌다고 말하는지**(1.26.123) — 날 판정은 **그날의 창**으로 재 자료를 더해도 안 움직이고(1.26.153, 예전엔 가장 넓은 날이 잣대라 좁은 날이 소급 탈락했다), 회차 판정은 그 시간대만 본다 · 창이 회차를 못 덮으면 밝히는지 · 반쪽짜리 날이 섞이면 관측시간과 결품시간이 함께 줄어드는지, 그리고 **어느 요일 구분으로 채점했는지**(1.26.127) — 실험은 `auto`(**돌린 날의 달력**)를 쓰고 있어 일요일에 평일 계획을 휴일 순수요로 재고 있었다 · 기록이 없으면 조용히 넘어가지 않는지. 여기서 나온 숫자가 논문에 실린다 |
...
| **합계** | **745** | 27개 파일 · 약 200초 (`python -m pytest`) |
