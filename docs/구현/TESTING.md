...
파이프라인이 아예 돌지 않았습니다. 그때 만든 안전망이 지금의 351개 테스트입니다.
이후 모든 수정은 이 위에서 이뤄졌습니다.

```powershell
python -m pytest              # 전체 351개 (약 50~80초)
...
| [tests/test_charts.py](../../tests/test_charts.py) | 16 | **그래프 규칙** — 계열 하나에 선 하나(축 둘 금지), 색을 SVG에 박지 않는지, 값 표시는 끝점만, 좌표가 뷰박스 안인지, 발산 척도의 가운데가 무채색인지 |
