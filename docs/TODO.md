...
- ~~`VEHICLE_TOTAL = 21` 미사용~~ → 해소. 회차당 투입 상한(`VEHICLES_PER_ROUND`)이
  클러스터 수를 제한하고, 실제 차량이 배정된다 ([FLEET.md](FLEET.md)).
- ILP는 25km/h·Haversine, VRP는 30km/h — 기준 통일 여부는 운영 데이터로 결정 필요.
- VRP 작업시간은 "자전거 1대당 30초" 가정 — 현장 값으로 검증 필요.
- **차량이 여러 클러스터를 순차 처리하지 못함** — 회차당 차량 1대 = 클러스터 1개 고정.
- **시간 예산이 제약이 아님** — 초과를 표시·경고하지만(1.8.1) 계획을 바꾸지는 않는다.
  실측상 소요시간의 70~80%가 이동이므로, step1 목적함수의 거리 가중치 `γ` 상향이
  가장 직접적인 개선책이다 ([FLEET.md](FLEET.md)의 시간 예산 절).
- `VEHICLES_PER_ROUND = 10`의 현장 근거 확인 필요.
...
12. **KPI 체계 구축**: 설계는 [KPI.md](KPI.md)에 완료. 도입 순서는
    ① `kpi_summary` 테이블 → ② 시간 예산 준수율·km당 개선량 → ③ `/kpi` 화면
    → ④ 결품 시간 시뮬레이션 → ⑤ 수요 예측 백테스트.
    **현재 `improvement_rate`가 계획 달성률이라 실측 성격의 지표가 없는 것이 핵심 공백.**

---

## ✅ 완료 (2026-08-11, 버전 1.9.1) — 실데이터 하루 3회차 검증
...
## ✅ 완료 (2026-08-11, 버전 1.9.0) — 실데이터 첫 전체 실행
