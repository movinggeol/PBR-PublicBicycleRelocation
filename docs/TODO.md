...
13. 🔴 **백테스트가 드러낸 문제 3가지** ([KPI.md](KPI.md) 3-E) — 지금 가장 중요한 과제:
    - **`z = 1.65`가 부족하다.** 목표 95% 커버리지인데 실측 91.7~92.8%.
      실측으로 구한 필요값은 **z ≈ 1.99**. `target_qty`가 체계적으로 조금씩 부족하다.
    - **`_10_15` 회차의 `mu`는 '0 예측'과 차이가 없다**(−0.2%). 이 시간대의
      `target_qty`는 근거가 약하므로 회차 자체를 재검토할 필요가 있다.
    - **계절 전환기가 약하다.** 2월→3월 커버리지 86.5%. 수요가 1.8배 뛰는 구간을
      한 달 전 평균이 못 따라간다.
