...
12. **KPI 체계 구축**: ①~④단계 완료(1.10.0). 남은 것은 ⑤ 수요 예측 백테스트.
    결품 시뮬레이션으로 실측 성격의 지표가 생겼지만, **`target_qty`의 근거인 `mu`가
    맞는지는 아직 검증되지 않았습니다** — ⑤가 그 공백입니다([KPI.md](KPI.md) 3-E).
    포화 시간·수요 충족률도 미구현입니다.
