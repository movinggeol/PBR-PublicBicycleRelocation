...
| 구분 | 상태 | 판정 |
| --- | --- | --- |
| 동작하는 시스템 | step0~4 실데이터 완주, 웹 대시보드, 테스트 238개 | ✅ 졸업작품으로 충분 |
...
| 평가의 결함 3건 | 계획 기준 지표·요일 필터 → ✅ 수정(1.18.3~4) / depot 복귀 누락 → ⏸ 보류 | 🟡 [TODO.md](TODO.md) P1 |
...
3회차·회차당 10대 로테이션·120분 예산·적재 10대)을 그대로 반영해 수요예측부터 경로까지
...
| 항목 | 현재 | 할 일 |
| --- | --- | --- |
| CI | 없음 | GitHub Actions로 `pytest` 자동 실행 — 테스트 238개의 신뢰도가 올라간다 |
| LICENSE | 없음 | 공개 저장소로 제출한다면 필요 |
| 재현 패키지 | `data/`가 gitignore라 외부인은 실행 불가 | `tools/make_sample_data.py` 기반 "5분 안에 돌려보기" 절차를 README에 |
...
- `VEHICLES_PER_ROUND = 10`, 작업시간 30초/대는 **현장 확인이 안 된 가정값**이다
...
**7번은 지금 고치지 않고 한계로 남깁니다** (2026-08-21 결정). 복귀 구간을 넣으면
예산 초과가 3건 → 13건(30개 중 43%)으로 드러나는데, '차량 1대 = 클러스터 1개'
구조에서는 클러스터를 더 쪼갤 수 없어 **작업량을 줄이거나 회차당 차량을 늘리는
운영 결정**이 필요합니다. 둘 다 현장 확인이 있어야 하는 사안이라, 논문에서는
**측정된 한계로 정직하게 쓰는 편**이 낫습니다 — 8장(한계)과 향후 과제로.

논문에 쓸 문장: *"경로 계산에 depot 복귀가 포함되지 않아 소요시간이 과소 추정되며,
이를 반영하면 30개 클러스터 중 13개가 120분 예산을 초과한다. 차량 1대가 클러스터
1개를 담당하는 현행 운영 모델에서는 클러스터 분할로 해결할 수 없어, 회차당 투입
대수 확대 또는 작업 대상 축소가 필요하다."*
