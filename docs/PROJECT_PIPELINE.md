...
│       ├── VRP/
│       └── 성능 지표/
├── step0 (raw데이터 처리)/                # 수집·전처리
├── step0(전처리 및 EDA)/                  # 이력 병합·탐색
├── step1 (작업대상 선정 및 클러스터링)/
├── step2 (ilp, vrp)/
├── step3 (결과 시각화)/
├── step4 (성과 지표)/
└── docs/
...
step0 (raw데이터 처리)/tashu_api.py가 TASHU API에서 대여소별 현재 재고를 조회합니다.
...
data/raw_data/에 타슈 대여 이력 CSV를 둡니다. 월별 파일은 step0(전처리 및 EDA)/concat_1year_file.py로 병합할 수 있습니다.
...
step2 (ilp, vrp)/ilp.py는 클러스터별 Pick 대여소에서 Drop 대여소로 이동할 자전거 수를 정수선형계획법으로 계산합니다.
...
step3 (결과 시각화)/module.py는 CSV 좌표와 경로를 정규화하고 TMAP 응답을 처리합니다. main.py는 VRP 경로, Pick·Drop 마커, 클러스터 정보를 Folium 지도에 표시합니다.
