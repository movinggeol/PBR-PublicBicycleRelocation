...
파이프라인이 아예 돌지 않았습니다. 그때 만든 안전망이 지금의 202개 테스트입니다.
이후 모든 수정은 이 위에서 이뤄졌습니다.

```powershell
python -m pytest              # 전체 202개 (약 50~80초)
...
| [tests/test_webapp.py](../tests/test_webapp.py) | 33 | 웹 라우트가 통째로 깨지는 사고 방지, 실행 폼 입력 검증, 최신 산출물 쪽 나눔 ([WEBAPP.md](WEBAPP.md)) |
