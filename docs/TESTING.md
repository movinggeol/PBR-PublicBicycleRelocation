...
파이프라인이 아예 돌지 않았습니다. 그때 만든 안전망이 지금의 299개 테스트입니다.
이후 모든 수정은 이 위에서 이뤄졌습니다.

```powershell
python -m pytest              # 전체 299개 (약 50~80초)
...
| [tests/test_calculations.py](../tests/test_calculations.py) | 45 | **계산 자체** — 목표재고 공식·군집 목적함수·VRP 적재/시간 제약·ILP 수급 제약·집행 기준 결품·**솔버 값 반올림/입력 방어** ([FORMULATION.md](FORMULATION.md)) |
