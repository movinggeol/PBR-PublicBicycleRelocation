...
파이프라인이 아예 돌지 않았습니다. 그때 만든 안전망이 지금의 175개 테스트입니다.
이후 모든 수정은 이 위에서 이뤄졌습니다.

```powershell
python -m pytest              # 전체 175개 (약 26~42초)
...
| [tests/test_day_type.py](../tests/test_day_type.py) | 24 | 평일/휴일 분리·공휴일 판정·수요 모델 폴백 ([steps/step0_raw.md](steps/step0_raw.md)) |
