...
# ---------------- 계절 수준 보정 (warmup) ----------------
# (season_ratio()가 아래 warmup_ratio()를 부른다 — 파이썬은 호출 시점에 이름을
#  찾으므로 정의 순서는 문제가 되지 않는다. apply_warmup()은 calculate_target_qty가
#  쓰고, build_features()는 이미 구한 배율만 인자로 받는다.)
#
...
# |mu| > 2인 대여소만 쓴다. REBAL_MIN_QTY(|rebal_qty| 문턱, 기본 2)와 숫자만 같을 뿐
# 연동되지 않으며(PBR_REBAL_MIN_QTY를 바꿔도 이 값은 그대로다), 실제 작업 대상을
# 정하는 것은 TOP_STATION_LIMIT다(project_config).
