...
def test_head_n이_곧_경과일_내림차순이다(bc):
    """🔴 **E-6이 새 정렬 규칙을 만들지 않는 근거.**

    `detect_broken()`은 `broken_at` 오름차순으로 돌려주므로, E-5가 정한 정렬
    (*마지막 대여 이후 경과일 내림차순*)은 곧 `head(N)`이다. 이 성질이 깨지면
    민감도 표가 **다른 코호트**를 재게 된다.
    """
    bikes = _bikes(50, station_span=5)
    assert bikes["broken_at"].is_monotonic_increasing

    앞 = bikes.head(10)
    assert 앞["broken_at"].max() <= bikes["broken_at"].iloc[10], (
        "head(N)이 가장 오래된 N대가 아니면 E-5의 정렬과 어긋난다")


def test_무릎은_하한에_처음_닿는_임계치다(bc):
    """무릎의 정의를 **결과를 보기 전에** 고정한다 — `depot왕복`이 물리적
    하한 ceil(N/용량)에 처음 도달하는 대수 임계치다(34장이 읽은 것과 같은 자).
    """
    표 = pd.DataFrame([
        {"정책": "5대", "방치평균_일": 0.2, "대당km": 6.0, "depot왕복": 20},
        {"정책": "10대", "방치평균_일": 0.5, "대당km": 4.7, "depot왕복": 10},
        {"정책": "20대", "방치평균_일": 1.2, "대당km": 4.1, "depot왕복": 10},
    ])
    assert bc.knee_threshold(표, size=100, capacity=10) == 10, (
        "하한 10에 처음 닿는 것은 임계치 10이다")
    # 하한에 닿는 행이 없으면 무릎도 없다 — 없는 것을 지어내지 않는다
    assert bc.knee_threshold(표.assign(depot왕복=99), size=100, capacity=10) is None


def test_짝짓기는_방치평균이_가장_가까운_쪽을_고른다(bc):
    """짝짓기 규칙이 결과를 따라가지 않도록 고정한다. 같은 거리면 value가
    작은 쪽이다 — 동점을 사람이 고르면 기준이 결과에 맞춰진다.
    """
    표 = pd.DataFrame([
        {"정책": "10대", "방치평균_일": 0.53, "대당km": 4.74, "depot왕복": 75},
        {"정책": "50대", "방치평균_일": 2.81, "대당km": 3.37, "depot왕복": 15},
        {"정책": "1일", "방치평균_일": 0.55, "대당km": 5.25, "depot왕복": 80},
        {"정책": "7일", "방치평균_일": 3.24, "대당km": 3.51, "depot왕복": 20},
    ])
    짝 = bc.pair_policies(표).set_index("일수")

    assert 짝.loc["1일", "대수"] == "10대"      # 0.55에 가장 가까운 것은 0.53
    assert 짝.loc["7일", "대수"] == "50대"
    assert set(짝["승자"]) == {"대수"}, "두 축 모두 낮으면 대수 승이다"


def test_한_축만_나으면_무승부다(bc):
    """🔴 **맞바꿈을 승리로 세지 않는다.** 34장의 '지배'는 *두 축 모두*라는
    뜻이고, 한 축만 나은 것을 승리로 세면 지배가 공짜로 성립한다.
    """
    표 = pd.DataFrame([
        {"정책": "10대", "방치평균_일": 0.40, "대당km": 6.00, "depot왕복": 75},
        {"정책": "1일", "방치평균_일": 0.55, "대당km": 5.25, "depot왕복": 80},
    ])
    assert bc.pair_policies(표)["승자"].iloc[0] == "무승부"


def test_일수가_두_축_모두_나으면_일수_승이다(bc):
    """반대쪽도 함께 고정한다 — 위 테스트가 공허해지지 않게."""
    표 = pd.DataFrame([
        {"정책": "10대", "방치평균_일": 0.80, "대당km": 6.00, "depot왕복": 75},
        {"정책": "1일", "방치평균_일": 0.55, "대당km": 5.25, "depot왕복": 80},
    ])
    assert bc.pair_policies(표)["승자"].iloc[0] == "일수"


def test_일수_승이_하나라도_있으면_지배를_철회한다(bc):
    """사전 등록한 갈래를 판정 함수가 그대로 따르는지 고정한다."""
    표 = pd.DataFrame([
        {"코호트": "정렬", "N": 745, "무릎": 10, "대수승": 4, "일수승": 0, "무승부": 0, "하루발생": 8.3},
        {"코호트": "정렬", "N": 100, "무릎": 10, "대수승": 3, "일수승": 1, "무승부": 0, "하루발생": 1.1},
    ])
    판정 = bc.cohort_verdict(표, capacity=10)
    assert "철회" in 판정
    assert "구조" in 판정, "모든 N에서 무릎이 용량이면 구조라고 말해야 한다"


def test_무작위_대조가_갈리면_정렬_기준_탓이라고_말한다(bc):
    """대조군이 장식이 되지 않게 고정한다."""
    표 = pd.DataFrame([
        {"코호트": "정렬", "N": 100, "무릎": 10, "대수승": 4, "일수승": 0, "무승부": 0, "하루발생": 1.1},
        {"코호트": "무작위(씨앗 1)", "N": 100, "무릎": 10, "대수승": 2, "일수승": 0, "무승부": 2, "하루발생": 1.1},
    ])
    assert "정렬 기준" in bc.cohort_verdict(표, capacity=10)

    같음 = 표.copy()
    같음.loc[1, "대수승"] = 4
    같음.loc[1, "무승부"] = 0
    assert "원인은 크기다" in bc.cohort_verdict(같음, capacity=10)
