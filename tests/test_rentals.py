...
PARKING = "step0 (raw데이터 처리)/extract_parking_lot.py"


@pytest.mark.parametrize("script, output, prerequisites", [
    ("step0 (raw데이터 처리)/raw_to_net.py", "순수요/st_net_daily ({period}).csv", []),
    # api_to_info는 주차대수 산출물이 먼저 있어야 한다
    ("step0 (raw데이터 처리)/api_to_info.py", "대여소 정보/st_info ({now}).csv", [PARKING]),
])
