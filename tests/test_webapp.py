...
def test_차례가_본문_안쪽_끝에만_머물지_않는다(client, monkeypatch):
    """🔴 처음 놓았을 때는 본문(1200px) 상자 **안쪽**에 갇혀 있었다(1.26.175) — 그
    바깥의 진짜 빈 공간(뷰포트 오른쪽 끝까지)은 그대로 버려두고 있었다
    (실측 1500px 화면: 차례 오른쪽 끝~뷰포트 끝이 174px 비어 있었다).

    그 여백의 절반만큼만 밀어 본 판(1.26.177)도 있었지만, 화면이 넓을수록
    본문도 차례도 못 쓰는 빈 띠가 둘 사이에 남아 **본문 글줄이 왼쪽으로
    쏠려 보인다**는 지적을 받았다(1.26.187, 사용자 확인). 지금은 차례를
    뷰포트 오른쪽 가장자리에 작은 틈(`--sp-lg`)만 두고 바짝 붙이고,
    본문의 예약 폭은 그만큼 **넓은 화면에서 0까지 줄어야 한다** —
    고정 px로 늘 같은 폭을 비워 두면 넓은 화면에서 그 폭이 통째로 낭비된다.
    """
    body = client.get("/kpi").text
    start = body.index(":root[data-has-toc] .toc {")
    assert "right: var(--sp-lg);" in body[start:start + 300], (
        "차례가 뷰포트 오른쪽 가장자리에 고정 틈으로 붙지 않는다")
    assert "max(0px, calc(600px + var(--sp-lg) + var(--toc-w) - 50vw))" in body, (
        "본문의 예약 폭이 넓은 화면에서 0으로 줄지 않거나, 차례의 오른쪽 틈(sp-lg)을 "
        "안 셈해 본문 글자와 차례가 겹친다")
